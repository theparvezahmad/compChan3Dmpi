...
logical 	:: myperiod, bottom, top, east, west, north, south, master
...
call mpi_cart_create(mpi_comm_world,3,[3,3,4],[.true.,.true.,.false.],.false.,comm3d,ierr)
...
if(master) then
	write(6,*) "=================================================================="
	write(6,*) "Compressible Channel Flow Solver started at :",dateTime()
	write(6,*) "=================================================================="
endif
...
select case(inputMode)
	...
	case(2)
		...
		WRITE(filename,'(a,i3.3,a)') '../../input/input364/3D',id+1,'.dat'
		...
		do k=bs(3),es(3)
			...
			do j=bs(2),es(2)
				read(10,*)
				do i=bs(1),es(1)
					read(10,101) x1(i),x2(j),x3(k),var(i,j,k,1),var(i,j,k,2),var(i,j,k,3),var(i,j,k,4)	&
																										 ,var(i,j,k,5),var(i,j,k,6),var(i,j,k,7)
					if(st/=0)	then
...
if(master) write(6,*) "Input file successfully read"
...
!~ 																					)*invdXi*invJac(k)*one12th
		...
		do j=bs(2),es(2)
			do i=bs(1),es(1)
				...
				if(bottom) then
					var(i,j,1,2:3) = 0.0d0
					var(i,j,1,4  ) = 0.03d0*sin(10.0d0*x1(i) - 3.0d0*time) ! Corresponds to third case S3 of Kalam
					var(i,j,1,6) = 1.0d0
	...
	if(mod(nstep,istat)==0) then
		...
		do k=1,nP(3)
			...
			if(k >= bs(3) .and. k <= es(3)) then
				do j=bs(2),es(2)
					do i=bs(1),es(1)
						loc(i,j,1:7)	= var(i,j,k,1:7) !quar*(var(i,j,k,1:7) + var(i+1,j,k,1:7) + var(i,j+1,k,1:7) + var(i+1,j+1,k,1:7) )
					enddo
				enddo				
			endif
