...
double precision			::tmpR1,tmpR2,tmpR3,tmpR4,tmpR5,tmpA1(5),loc(nP(1),nP(2),7)
...
double precision			::dudxdz,dvdydz,d2wdz2,drhow2dz,lhs,omega
integer,allocatable,dimension(:,:) 	::tArr,bb,ee

!------------------------MPI Variables----------------------------------
integer,dimension(3)		:: bs,es,bn,en,siz,des,src,myDim,myCoord
integer		:: liney,yz1p, xz1p, xy1p, yz1p5v, xz1p5v, xy1p5v, yz2p7v, xz2p7v, xy2p7v
...
double precision,dimension(0:nP(1)+1,0:nP(2)+1,0:nP(3)+1,7)		:: var ! In order of rho,U,V,W,E,T,P
...
integer,dimension(nP(1),nP(2),nP(3))													:: pattern
...
!---------------------------------------------------------------------
call mpi_sendrecv(var(en(1)-1,bn(2),bn(3),1),1,yz2p7v,des(1),50,var(bn(1)-2,bn(2),bn(3),1),1,yz2p7v,src(1),50,mpi_comm_world,STATUS,ierr)
call mpi_sendrecv(var(bn(1)  ,bn(2),bn(3),1),1,yz2p7v,src(1),50,var(en(1)+1,bn(2),bn(3),1),1,yz2p7v,des(1),50,mpi_comm_world,STATUS,ierr)

call mpi_sendrecv(var(bn(1),en(2)-1,bn(3),1),1,xz2p7v,des(2),50,var(bn(1),bn(2)-2,bn(3),1),1,xz2p7v,src(2),50,mpi_comm_world,STATUS,ierr)
call mpi_sendrecv(var(bn(1),bn(2)  ,bn(3),1),1,xz2p7v,src(2),50,var(bn(1),en(2)+1,bn(3),1),1,xz2p7v,des(2),50,mpi_comm_world,STATUS,ierr)

call mpi_sendrecv(var(bn(1),bn(2),en(3)-1,1),1,xy2p7v,des(3),50,var(bn(1),bn(2),bn(3)-2,1),1,xy2p7v,src(3),50,mpi_comm_world,STATUS,ierr)
call mpi_sendrecv(var(bn(1),bn(2),bn(3)  ,1),1,xy2p7v,src(3),50,var(bn(1),bn(2),en(3)+1,1),1,xy2p7v,des(3),50,mpi_comm_world,STATUS,ierr)
	...
	do iter=1,maxiter
		...
		do j=bs(2),es(2)
			do i=bs(1),es(1)
				...
				if(bottom) then
					omega=3.0d0
					tmpR1=x2(nP(2))/10.0d0
					tmpR2=sqrt((x1(i)-two*tmpR1)**two + (x2(j)-2.0d0*tmpR1)**two)
					tmpR3=sqrt((x1(i)-two*tmpR1)**two + (x2(j)-5.0d0*tmpR1)**two)
					tmpR4=sqrt((x1(i)-two*tmpR1)**two + (x2(j)-8.0d0*tmpR1)**two)
					
					if(tmpR2 < tmpR1) then
						pattern(i,j,1) = 1
						tmpR5=myatand(x1(i)-two*tmpR1,x2(j)-2.0d0*tmpR1)
						var(i,j,1,2) = -tmpR2*omega*dsin(tmpR5)
						var(i,j,1,3) = +tmpR2*omega*dcos(tmpR5)
						
					elseif(tmpR3 < tmpR1) then
						pattern(i,j,1) = 2
						tmpR5=myatand(x1(i)-two*tmpR1,x2(j)-5.0d0*tmpR1)
						var(i,j,1,2) = -tmpR3*omega*dsin(tmpR5)
						var(i,j,1,3) = +tmpR3*omega*dcos(tmpR5)
						
					elseif(tmpR4 < tmpR1) then
						pattern(i,j,1) = 3
						tmpR5=myatand(x1(i)-two*tmpR1,x2(j)-8.0d0*tmpR1)
						var(i,j,1,2) = -tmpR4*omega*dsin(tmpR5)
						var(i,j,1,3) = +tmpR4*omega*dcos(tmpR5)
					else
						pattern(i,j,1) = 0
						var(i,j,1,2) = zer
						var(i,j,1,3) = zer
